...
Next recover sparse compiler input without executing or flattening game bytes; investigate the nine specific boundaries above; extend exact callback provenance and indirect tables/vtables while retaining mutable/unknown targets. Actual startup still needs native import, callback, return, exception and service contracts before the P3/P4 gates pass.

P1 remains independent: Hunter's Dream route, profiler overhead, CPU/GPU/queue costs and the opening audio mutex crash remain pending. No new baseline run occurred in this continuation. Ordinary shell commands now work under the updated unrestricted, never-ask permission policy; do not pass sandbox_permissions or request another restart/read approval. No user action or paid component is needed.
