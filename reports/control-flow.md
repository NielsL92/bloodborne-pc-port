...
Current source-exit compilation (2026-09-06 21:35 UTC): reports/source-exits.md and its evidence establish the repeated complete 386-object replacement set, exact 874,279 compiler instruction bytes, all 4,305 sourced nonreturn assertions, 551 classified source/target pairs and 182 runtime import stubs. Use source-exit-manifest-v2-regression/active-objects.json and compiler v11-source-exits / semantics v35-divide. Saved bitcode is pre-clang-O2; its structural CFG counts are not native execution coverage. Native handler/binding/linkage work remains, so P3 and native startup are open. Earlier dated checkpoints below are historical.
...
X87 metadata checkpoint (2026-09-06 14:54 UTC): reports/x87-metadata.md/evidence preserves unified producer metadata, FNINIT selector reset and explicit comparison C1/OF/SF/AF policies. Selected-policy tests pass; deviations from this Intel host remain counted. Accepted startup and unresolved native control census are unchanged.

X87 store checkpoint (2026-09-06 15:14 UTC): reports/x87-stores.md/evidence preserves bounded numeric stores, instruction-level exception/write/pop priority, dense rounding failures and reproducible corrected outputs. Experimental v29 / compiler v6 remains separate from accepted startup v9 / compiler v5. Arithmetic, FSCALE, MMX and native control contracts remain open; no game execution is inferred.

Current-graph x87/MMX checkpoint (2026-09-06 15:25 UTC): reports/x87-remaining.md/evidence identifies nine arithmetic/FSCALE sites and no MMX in the recovered graph. Experimental compiler v7 fails closed on future MMX/EMMS/FEMMS; shared-state support is deferred until such code is discovered. Unknown targets and runtime/control gates remain unchanged; static absence is not execution coverage.
