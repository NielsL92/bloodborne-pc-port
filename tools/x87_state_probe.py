"""Expose x87 State discrepancies with authored AOT and hardware sequences."""
...
cases=[('push-one',['fld1']),('push-zero',['fldz']),('push-pair',['fld1','fldz']),('exchange-pop',['fld1','fldz','fxch st(1)','fstp st(0)']),('initialize',['fninit']),('load-control',['fldcw word ptr [ARG]']),('store-control',['fnstcw word ptr [ARG]']),('pop',['fstp st(0)']),('compare-pop',['fucomip st(0), st(1)']),('store-status',['fnstsw word ptr [ARG]'])]
...
for n,(name,instructions) in enumerate(cases):
 ...
 hardware.extend([f'.globl hw_{n}',f'hw_{n}:','fxsave64 [r8]','fxrstor64 [rcx]',*[i.replace('ARG','r9') for i in instructions],'fxsave64 [rdx]','fnstenv [rdx+512]','fxrstor64 [r8]','ret'])
...
run('fixture',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/DADDRESS_SIZE_BITS=64','/DHAS_FEATURE_AVX=1','/DHAS_FEATURE_AVX512=0','/clang:-mlong-double-80','/clang:-mno-avx','/I'+str(ROOT/'external/remill/include'),'/I'+str(out),'/c',ROOT/'native/semantics/x87_state_probe.cpp','/Fo'+str(out/'fixture.obj')])
...
observations=[json.loads(line) for line in (out/'execute.stdout').open()];assert len(observations)==11520
counts={};examples={}
for r in observations:
 for finding in r['differences']:
  key=specs[r['form']]['name']+':'+finding;counts[key]=counts.get(key,0)+1;examples.setdefault(key,r)
write_json(out/'summary.json',dict(status='characterization complete; semantic correctness is not established',cases=len(observations),differing_cases=sum(bool(r['differences']) for r in observations),differences=counts,examples=examples,raw_sha256=sha(out/'execute.stdout'),lifter_sha256=sha(a.lifter),semantics_sha256=sha(a.semantics/'amd64_avx.bc'),fixture_sha256=sha(out/'fixture.exe'),limitations='Authored code only; all exceptions masked; tag/control/defined status and nonempty 80-bit register payloads compared. Full State and guest exception delivery remain unverified. Hardware instruction/data pointers and reserved bits are excluded. FNINIT is a source regression case, absent from current recovered startup instructions.'))
print(json.dumps(dict(cases=len(observations),differences=counts)),flush=True)
