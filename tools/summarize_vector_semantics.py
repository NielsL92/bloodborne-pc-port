"""Audit constructor compilation, vector contracts, repeatability and remaining rejects."""
...
base=root/'local/compiler-spike/startup-batch-all-v1';left_dir=root/'local/compiler-spike/startup-vector-recompile-v1';right_dir=root/'local/compiler-spike/startup-vector-recompile-v2-repeat'
...
assert len(new_units)==17 and len(objects)==8 and len(traps)==16 and len(failures)==31 and not new_units&old_units
sem_dir=root/'build/extended-semantics-v3-blend-rsqrt';sem=read(sem_dir/'identity.json');sem_hash=sha(sem_dir/'amd64_avx.bc');assert sem_hash==sem['semantics_sha256']
...
checks={}
for family,directory,count in [('shuffle','shuffle-semantics-v2-combined',32768),('blend','blend-semantics-v1',122880),('rsqrt','rsqrt-semantics-v1',399360),('bmi','bmi-vector-regression-v1',34816)]:
...
runs={}
for suffix in ('build-shuffle-semantics-v1','shuffle-semantics-checks-v1','bmi-shuffle-regression-v1','startup-shuffle-recompile-v1','build-extended-semantics-v3-blend-rsqrt','blend-semantics-v1','rsqrt-semantics-v1','shuffle-semantics-v2-combined','bmi-vector-regression-v1','startup-vector-recompile-v1','startup-vector-recompile-v2-repeat'):
 name='20260905-p3-'+suffix;p=root/'local/runs'/name;m=read(p/'manifest.json');assert m['status']==('failed' if suffix.startswith('startup-') else 'pass')
