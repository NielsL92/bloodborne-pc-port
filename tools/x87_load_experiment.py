"""Validate scoped native x87 loads and precise stack/memory/fault boundaries."""
...
cases=[('load32',['fld dword ptr [ARG]']),('load64',['fld qword ptr [ARG]']),('loadi16',['fild word ptr [ARG]']),('loadi32',['fild dword ptr [ARG]']),('loadi64',['fild qword ptr [ARG]'])]
cases.extend((cases[i][0]+'-wait',cases[i][1]+['fwait']) for i in range(5))
cases.extend((cases[i][0]+'-store80',cases[i][1]+['fstp tbyte ptr [ARG]']) for i in range(5))
...
header.append('static unsigned base_forms[]={'+','.join(map(str,list(range(5))*3))+'};')
...
run('fixture',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/DADDRESS_SIZE_BITS=64','/DHAS_FEATURE_AVX=1','/DHAS_FEATURE_AVX512=0','/clang:-mlong-double-80','/clang:-mno-avx','/I'+str(ROOT/'external/remill/include'),'/I'+str(out),'/c',ROOT/'native/semantics/x87_load_fixture.cpp','/Fo'+str(out/'fixture.obj')])
...
summary=dict(status='experimental scoped x87 load characterization complete',cases=sum(r['cases'] for r in observations),differing_cases=sum(r['differing_cases'] for r in observations),groups=observations,library_sha256=sha(a.library),raw_sha256=sha(out/'execute.stdout'),lifter_sha256=sha(a.lifter),semantics_sha256=sha(a.semantics/'amd64_avx.bc'),fixture_sha256=sha(out/'fixture.exe'),limitations='Authored AOT and Intel hardware only; no game bytes. Narrow floating and signed integer loads use scoped native SoftFloat conversions. Undefined C0/C2/C3 are excluded from the hardware status oracle. Pointer/opcode profiles are explicit. Numeric stores, arithmetic, older producer metadata and MMX coherence remain open.')
write_json(out/'summary.json',summary)
print(json.dumps({k:v for k,v in summary.items() if k!='groups'}),flush=True)
assert summary['differing_cases']==0, 'x87 load contract differences retained in summary.json'
