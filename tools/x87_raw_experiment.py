"""Validate raw x87 transfers, conditionals and precise pending-fault boundaries."""
...
cases=[('load80',['fld tbyte ptr [ARG]']),('store80',['fstp tbyte ptr [ARG]']),('load-st0',['fld st(0)']),('load-st1',['fld st(1)']),('change-sign',['fchs']),('conditional-ne',['fcmovne st(0), st(1)']),('roundtrip',['fld tbyte ptr [ARG]','fstp tbyte ptr [ARG]']),('register-sign-store',['fld st(1)','fchs','fstp tbyte ptr [ARG]'])]
cases.extend((cases[i][0]+'-wait',cases[i][1]+['fwait']) for i in range(6))
...
header.append('static unsigned base_forms[]={'+','.join(map(str,list(range(8))+list(range(6))))+'};')
...
run('fixture',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/DADDRESS_SIZE_BITS=64','/DHAS_FEATURE_AVX=1','/DHAS_FEATURE_AVX512=0','/clang:-mlong-double-80','/clang:-mno-avx','/I'+str(ROOT/'external/remill/include'),'/I'+str(out),'/c',ROOT/'native/semantics/x87_raw_fixture.cpp','/Fo'+str(out/'fixture.obj')])
...
summary=dict(status='experimental raw x87 instruction characterization complete',cases=sum(r['cases'] for r in observations),differing_cases=sum(r['differing_cases'] for r in observations),groups=observations,raw_sha256=sha(out/'execute.stdout'),lifter_sha256=sha(a.lifter),semantics_sha256=sha(a.semantics/'amd64_avx.bc'),fixture_sha256=sha(out/'fixture.exe'),limitations='Authored AOT and Intel hardware only; no game bytes. Raw80 transfers preserve unsupported encodings without invoking numeric conversion. Undefined C0/C2/C3 are excluded from the hardware status oracle. Pointer metadata is an explicit runtime service keyed by decoded PC, with no general guest mapping or PS4 fault delivery claim. Other x87 producer metadata, numeric conversions/arithmetic and MMX coherence remain open.')
write_json(out/'summary.json',summary)
print(json.dumps({k:v for k,v in summary.items() if k!='groups'}),flush=True)
assert summary['differing_cases']==0, 'raw x87 contract differences retained in summary.json'
