"""Validate native mutex lifecycle, ownership and thread exclusion through authored AOT calls."""
import argparse,json,subprocess
from pathlib import Path
from tools.cfg_recover_startup import sha,write_json
from tools.dev import ROOT,LLVM,environment
p=argparse.ArgumentParser();p.add_argument('out',type=Path);a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False);out=a.out.resolve();env=environment();steps=[];pc=0x100200000
# Indirect calls carry canonical logical service identifiers; no 32-bit displacement assumption.
body=[]
for i,target in enumerate([0x900001000+i*16 for i in range(8)]):
...
write_json(out/'input.json',dict(native_memory_provenance=True,roots=[pc+i*0x100 for i in range(8)],instructions=body))
header=[f'extern \"C\" Memory* sub_{pc+i*256:x}(State*,uint64_t,Memory*);' for i in range(8)];header.append('#define MUTEX_TARGETS {'+','.join('{'+str(pc+i*256)+'ULL,sub_'+format(pc+i*256,'x')+'}' for i in range(8))+'}');(out/'mutex-entries.h').write_text(chr(10).join(header)+chr(10),encoding='utf-8')
...
for name in ['fault','memory','control','intrinsics','fp','sourced','mutexattr','mutex','mutex_fixture']:
...
run('link',[LLVM/'bin/clang-cl.exe','/nologo',*objects,'/Fe'+str(out/'fixture.exe')]);run('positive',[out/'fixture.exe','positive']);positive=json.loads((out/'positive.stdout').read_text());assert positive['lifecycles']==4096 and positive['threaded_increments']==1536 and positive['recursive_waiter_checked']
for mode,reason in [('bad-pointer',4),('missing-provider',62),('double-init',59),('opaque-read',4),('static-init',60),('name',57),('normal-self',56)]:
...
result=dict(status='native mutex lifecycle ownership and threaded AOT checks pass',positive=positive,negative_stops=7,objects={p.name:sha(p) for p in objects},lifter_sha256=sha(lifter),semantics_sha256=sha(semantics/'amd64_avx.bc'),game_aot_execution=False,limits=['Authored native threading and stateful service checks only; no game gateway is bound by this fixture.','Opaque logical identifiers are not host pointers or guest-readable mutex layouts. Direct dereference remains an explicit unsupported memory stop.','Live-resource and monotonically increasing identifier bounds are native implementation limits, not console resource limits.']);write_json(out/'summary.json',result);print(json.dumps(result),flush=True)
