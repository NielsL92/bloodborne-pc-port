...
p=argparse.ArgumentParser();p.add_argument('first',type=Path);p.add_argument('repeat',type=Path);p.add_argument('out',type=Path);p.add_argument('--batch',nargs=2,type=Path,action='append',default=[]);p.add_argument('--cfg',nargs=2,type=Path,action='append',default=[]);p.add_argument('--recovery',nargs=2,type=Path,action='append',default=[]);p.add_argument('--chain',type=Path,action='append',default=[]);p.add_argument('--run',action='append',default=[]);a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
...
chains=[]
for folder in a.chain:
 chain=read(folder/'summary.json')
 for step in chain['advances']:
  current=Path(step['current']);assert sha(current/'summary.json')==step['summary_sha256']
  for name,digest in step['artifacts'].items():assert sha(current/name)==digest
  compiled=Path(step['compiled']);b=read(compiled/'summary.json');validate(b,compiled)
  for name,digest in {**b['files'],**b['native_objects']}.items():assert sha(compiled/name)==digest
 for step in chain['steps']:
  assert sha(Path('local/runs')/step['run']/'manifest.json')==step['manifest_sha256'];a.run.append(step['run'])
 chains.append(chain)
recorders=[]
for run in dict.fromkeys(a.run):
