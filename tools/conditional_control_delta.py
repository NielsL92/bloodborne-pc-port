...
SPECS=[dict(module=h,entry=e,site=pc,id='disputed-ending-'+h[:12]+'-'+format(e,'x')) for h,e,pc,t in SITES]
...
def main():
 ...
 with (a.before/'compilation-manifest.jsonl').open(encoding='utf-8') as old,(a.after/'compilation-manifest.jsonl').open(encoding='utf-8') as new:
  for x,y in zip(old,new,strict=True):
   ...
   assert {e['kind'] for e in added}=={'unresolved_conditional_control_continuation','annotated_control_contract_requires_runtime'} and len(added)==2
 ...
 assert {(r['module'],r['start']) for r in changed}==set(expected) and units==21181
 ...
 # Every existing unresolved target/annotation count is unchanged except the five explicitly checked conditional endings.
 ...
 assert frontier_delta==dict(fallthrough_at_fence=-5,annotated_control_contract_requires_runtime=5,unresolved_conditional_control_continuation=5),frontier_delta
 result=dict(status='exact five-site conditional-control recovery delta passed',before=str(a.before),after=str(a.after),source_db_sha256=sha(a.before/'analysis.sqlite'),target_db_sha256=sha(a.after/'analysis.sqlite'),target_manifest_sha256=sha(a.after/'compilation-manifest.jsonl'),changed=changed,frontier_delta=frontier_delta,units=units,remaining_quarantines=0,limitations='Only five exact conditional no-ordinary-return ending sites changed. Unknown indirect and callback targets, exception paths and every old contract obligation remain.')
 write_json(a.out/'delta.json',result);write_json(a.out/'entries.json',[dict(module=r['module'],start=r['start']) for r in changed])
 assert after['counts']['recovery_entry']==21181 and after['counts']['recovery_instruction']==874279 and after['counts']['recovery_issue']==0
