"""Validate scoped x87 stores and precise completion/exception boundaries."""
...
cases=[('store32',['fst dword ptr [ARG]']),('store64',['fst qword ptr [ARG]']),('store-pop32',['fstp dword ptr [ARG]']),('store-pop64',['fstp qword ptr [ARG]']),('trunc16',['fisttp word ptr [ARG]']),('trunc32',['fisttp dword ptr [ARG]']),('trunc64',['fisttp qword ptr [ARG]'])]
cases.extend((cases[i][0]+'-wait',cases[i][1]+['fwait']) for i in range(7))
cases.append(('store32-store64',['fst dword ptr [ARG]','fst qword ptr [ARG]']))
...
header.append('static unsigned base_forms[]={'+','.join(map(str,list(range(7))*2+[14]))+'};')
...
with (out/'x87-entries.h').open('a',encoding='utf-8') as h:
 ...
 for key,values in [('step_pcs',[[i['address'] for i in rows if s['pc']<=i['address']<s['pc']+64][:-1] for s in specs]),('step_fops',[[int(i['bytes'][0:2],16)%8*256+int(i['bytes'][2:4],16) if len(i['bytes'])>=4 else 0 for i in rows if s['pc']<=i['address']<s['pc']+64][:-1] for s in specs]),('step_kinds',[[0 if x=='fwait' else 2 for x in s['instructions']] for s in specs])]:
...
run('fixture',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/DADDRESS_SIZE_BITS=64','/DHAS_FEATURE_AVX=1','/DHAS_FEATURE_AVX512=0','/clang:-mlong-double-80','/clang:-mno-avx','/I'+str(ROOT/'external/remill/include'),'/I'+str(out),'/c',ROOT/'native/semantics/x87_store_fixture.cpp','/Fo'+str(out/'fixture.obj')])
...
summary=dict(status='experimental scoped x87 store characterization complete',cases=sum(r['cases'] for r in observations),differing_cases=sum(r['differing_cases'] for r in observations),groups=observations,library_sha256=sha(a.library),raw_sha256=sha(out/'execute.stdout'),lifter_sha256=sha(a.lifter),semantics_sha256=sha(a.semantics/'amd64_avx.bc'),fixture_sha256=sha(out/'fixture.exe'),limitations='Authored AOT and Intel hardware only; no game bytes. Floating stores and truncating integer stores use scoped native SoftFloat conversions; completion and pending exceptions are instruction-level contracts. Undefined C0/C2/C3 are excluded from the hardware status oracle. Pointer/opcode profiles are explicit. Arithmetic, FSCALE and MMX coherence remain open.')
write_json(out/'summary.json',summary)
print(json.dumps({k:v for k,v in summary.items() if k!='groups'}),flush=True)
assert summary['differing_cases']==0, 'x87 store contract differences retained in summary.json'
