...
for folder in a.supplement:
 extension=read(folder/'manifest.json');assert extension['schema']==1 and extension['base_registry_identity']==contract['registry_identity'];assert not extension['source_pairs'] and not extension['imports'];validation_path=Path(extension['validation_path']);validation=read(validation_path);assert sha(validation_path)==extension['validation_sha256'];assert validation['base_registry_identity']==contract['registry_identity'] and validation['compiled_roots']==extension['compiled_roots'];count=len(extension['compiled_roots']);assert 1<=count<=16;assert validation.get('validation_kind')=='conditional-simple-leaf-batch' or count==1;assert validation.get('cases_per_root',3072)==3072 and validation['positive']['cases']==3072*count and validation['negative_stops']==2*count;assert sha(extension['module']['path'])==extension['module']['module']
