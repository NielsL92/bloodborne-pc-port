...
p=argparse.ArgumentParser();p.add_argument('contract',type=Path);p.add_argument('out',type=Path);p.add_argument('--plan',type=Path,default=Path('local/runtime/loader-plan-v8-memory'));p.add_argument('--replay-seed',type=Path);p.add_argument('--trace-calls',action='store_true');p.add_argument('--mutex-attributes',type=Path);a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False);out=a.out.resolve();env=environment();steps=[]
...
for key,value in dict(ENTRY_PC=contract['entry_pc'],INITIAL_RSP=initial_rsp,PARAMETERS=contract['parameter_base'],EXIT_CALLBACK_PC=contract['exit_callback_pc'],EXPECTED_MAPPED_BYTES=expected_image['mapped_bytes'],CANARY_ENABLED=int(bool(providers)),CANARY_PC=providers.get('canary',{}).get('logical_address',0),CONTROL_TRACE=int(a.trace_calls),MUTEX_ATTRIBUTES_ENABLED=int(bool(service_contract))).items():header.append(f'static constexpr uint64_t {key}={value}ULL;')
...
native=[]
for name in ['fault','memory','control','intrinsics','fp','sourced','loader','canary','mutexattr','startup_fixture','registry']:
