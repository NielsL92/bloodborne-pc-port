...
for key,value in dict(ENTRY_PC=contract['entry_pc'],INITIAL_RSP=initial_rsp,PARAMETERS=contract['parameter_base'],EXIT_CALLBACK_PC=contract['exit_callback_pc'],EXPECTED_MAPPED_BYTES=expected_image['mapped_bytes'],CANARY_ENABLED=int(bool(providers)),CANARY_PC=providers.get('canary',{}).get('logical_address',0),CONTROL_TRACE=int(a.trace_calls),MUTEX_ATTRIBUTES_ENABLED=int(bool(service_contract)),MUTEXES_ENABLED=int(bool(service_contract and service_contract.get('mutex_services'))),RWLOCKS_ENABLED=int(bool(service_contract and service_contract.get('rwlock_services'))),DIRECT_MEMORY_BUDGET=service_contract.get('direct_memory_budget',0) if service_contract else 0).items():header.append(f'static constexpr uint64_t {key}={value}ULL;')
...
native=[]
for name in ['fault','memory','control','intrinsics','fp','sourced','loader','canary','mutexattr','mutex','direct_memory','rwlock','startup_fixture','registry']:
