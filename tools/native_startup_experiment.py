...
for folder in a.supplement:
 extension=read(folder/'manifest.json');assert extension['schema']==1 and extension['base_registry_identity']==contract['registry_identity'];assert not extension['source_pairs'] and not extension['imports'];validation_path=Path(extension['validation_path']);validation=read(validation_path);assert sha(validation_path)==extension['validation_sha256'];assert validation['base_registry_identity']==contract['registry_identity'] and validation['compiled_roots']==extension['compiled_roots'];validate(validation,validation_path.parent);assert sha(extension['module']['path'])==extension['module']['module']
 ...
 audit=read(validation_path.parent/'audit.json');assert audit['compiled_roots']==extension['compiled_roots'] and audit['native_memory_provenance'];assert (not audit['missing_instruction_starts'] or validation.get('validation_kind')=='independent-cfg-compilation') and not audit['unvisited_manifest_instructions'];assert len(set(extension['compiled_roots']))==len(extension['compiled_roots']) and not set(extension['compiled_roots'])&{r['pc'] for r in targets+imports}
