"""Validate scoped register x87 arithmetic and precise completion boundaries."""
import argparse,json,struct,subprocess
from pathlib import Path
from capstone import Cs,CS_ARCH_X86,CS_MODE_64
from tools.cfg_recover_startup import sha,write_json
from tools.dev import ROOT,LLVM,environment
p=argparse.ArgumentParser();p.add_argument('out',type=Path);p.add_argument('lifter',type=Path);p.add_argument('semantics',type=Path);p.add_argument('library',type=Path);p.add_argument('--prepare-only',action='store_true');a=p.parse_args()
out=a.out.resolve();out.mkdir(parents=True,exist_ok=False);env=environment();steps=[]
cases=[];indices=[]
for family in range(5):
 for index in range(8):
  ins=[f'faddp st({index}), st(0)',f'fsubp st({index}), st(0)',f'fsubrp st({index}), st(0)',f'fsub st(0), st({index})',f'fmul st(0), st({index})'][family]
  cases.append((f'arithmetic-{family}-st{index}',[ins]));indices.append(index)
for n in [family*8+index for family in range(5) for index in (1,2,7)]:
 cases.append((cases[n][0]+'-wait',cases[n][1]+['fwait']));indices.append(indices[n])
cases.extend([('multiply-subtract-pop',['fmul st(0), st(2)','fsubp st(1), st(0)']),('add-pop-subtract-reverse',['faddp st(2), st(0)','fsubrp st(1), st(0)'])]);indices.extend([2,2])
...
header.append('static unsigned source_indices[]={'+','.join(map(str,indices))+'};')
...
with (out/'x87-entries.h').open('a',encoding='utf-8') as h:
 ...
 for key,values in [('step_pcs',[[i['address'] for i in rows if s['pc']<=i['address']<s['pc']+64][:-1] for s in specs]),('step_fops',[[int(i['bytes'][0:2],16)%8*256+int(i['bytes'][2:4],16) if len(i['bytes'])>=4 else 0 for i in rows if s['pc']<=i['address']<s['pc']+64][:-1] for s in specs]),('step_kinds',[[0 if x=='fwait' else 3 for x in s['instructions']] for s in specs])]:
  h.write('static uint64_t '+key+'[][4]={'+','.join('{'+','.join(map(str,row))+'}' for row in values)+'};\n')
with (out/'x87-entries.h').open('a',encoding='utf-8') as h:h.write('static unsigned store_widths[][4]={'+','.join('{'+','.join(str(0 if x=='fwait' else 2 if ' word ' in x else 4 if 'dword' in x else 8) for x in s['instructions'])+'}' for s in specs)+'};\n')
...
run('fixture',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/DADDRESS_SIZE_BITS=64','/DHAS_FEATURE_AVX=1','/DHAS_FEATURE_AVX512=0','/clang:-mlong-double-80','/clang:-mno-avx','/I'+str(ROOT/'external/remill/include'),'/I'+str(out),'/c',ROOT/'native/semantics/x87_arithmetic_fixture.cpp','/Fo'+str(out/'fixture.obj')])
...
run('scope',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/clang:-mno-incremental-linker-compatible','/I'+str(ROOT/'external/SoftFloat-3e/source/include'),'/c',ROOT/'native/semantics/x87_numeric_scope_fixture.cpp','/Fo'+str(out/'scope.obj')])
run('link',[LLVM/'bin/clang-cl.exe','/nologo',out/'fixture.obj',out/'function.obj',out/'hardware.obj',out/'numeric.obj',out/'scope.obj',a.library.resolve(),'/Fe'+str(out/'fixture.exe')])
run('execute',[out/'fixture.exe'])
observations=[json.loads(line) for line in (out/'execute.stdout').open()];assert len(observations)==len(cases)+1
summary=dict(status='experimental register x87 arithmetic characterization complete',cases=sum(r['cases'] for r in observations),differing_cases=sum(r['differing_cases'] for r in observations),groups=observations,library_sha256=sha(a.library),raw_sha256=sha(out/'execute.stdout'),lifter_sha256=sha(a.lifter),semantics_sha256=sha(a.semantics/'amd64_avx.bc'),fixture_sha256=sha(out/'fixture.exe'),limitations='Authored AOT and Intel hardware only; no game bytes. Add/subtract/multiply use scoped native SoftFloat numeric helpers; stack, completion, pending exceptions and reserved-control rejection are instruction-level contracts. Undefined C0/C2/C3 are excluded from the hardware status oracle. Pointer/opcode profiles are explicit. FSCALE and native runtime integration remain open; absent MMX is explicitly rejected by the compiler.')
write_json(out/'summary.json',summary)
print(json.dumps({k:v for k,v in summary.items() if k!='groups'}),flush=True)
assert summary['differing_cases']==0, 'x87 arithmetic contract differences retained in summary.json'
