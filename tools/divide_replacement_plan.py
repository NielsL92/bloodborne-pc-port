"""Select every complete retained object containing a recovered DIV/IDIV site."""
...
ops=['div','idiv'];sites=[dict(module=m,rva=r,mnemonic=n,bytes=b,expected_implementation='BB_IDIV' if n=='idiv' else 'BB_DIV') for m,r,n,b in db.execute('SELECT module,rva,mnemonic,bytes FROM recovery_instruction WHERE mnemonic IN ('+','.join('?' for _ in ops)+') ORDER BY module,rva',ops)]
...
assert set(covered)==set(lookup) and len(sites)==28
