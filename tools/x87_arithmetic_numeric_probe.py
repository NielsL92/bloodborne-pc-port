...
cases=['faddp st(1), st(0)','fsubp st(1), st(0)','fsubrp st(1), st(0)','fsub st(0), st(1)','fmul st(0), st(1)'];assembly=['.intel_syntax noprefix','.text','.globl save_host','save_host:','fxsave64 [rcx]','ret'];header=[]
...
run('fixture',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/clang:-mno-avx','/I'+str(out),'/I'+str(ROOT/'external/SoftFloat-3e/source/include'),'/c',ROOT/'native/semantics/x87_arithmetic_numeric_fixture.cpp','/Fo'+str(out/'fixture.obj')])
...
rows=[json.loads(line) for line in (out/'execute.stdout').read_text(encoding='utf-8').splitlines()];assert len(rows)==5
summary=dict(status='scoped arithmetic numeric characterization retained',cases=sum(r['cases'] for r in rows),differing_cases=sum(r['differing_cases'] for r in rows),reserved_control_cases=sum(r['reserved_control_cases'] for r in rows),groups=rows,raw_sha256=sha(out/'execute.stdout'),library_sha256=sha(library),fixture_sha256=sha(out/'fixture.exe'),source_sha256=sha(ROOT/'native/semantics/x87_arithmetic_numeric_fixture.cpp'),limitations='Authored hardware versus ordinary native numeric helpers only; no AOT or game execution. Valid precision modes compare raw results/flags/C1 and scoped library/host state. Reserved precision-control 1 returns an explicit unsupported status rather than implicitly adopting host behavior. Instruction stack/tag/control boundaries remain separate.')
