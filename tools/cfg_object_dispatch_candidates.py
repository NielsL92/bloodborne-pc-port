...
p=argparse.ArgumentParser();p.add_argument('source',type=Path);p.add_argument('out',type=Path);p.add_argument('--nested-lock',action='store_true');p.add_argument('--subobject',action='store_true');a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
...
entries=[0xf91390,0xf95810,0xfa6410,0x2ba2b10,0x207fa50,0x20819f0]+([0x207f070] if a.nested_lock else [])+([0x207bbf0,0x1251290,0x15cb9a0] if a.subobject else []);witnesses=[];selection=[]
...
if a.nested_lock:specs.append(dict(name='factory-lock-nested-slot-10',assignment_entry=0x207fa50,lea=0x207fab1,adjust=0x207fab8,store=0x207fabc,call_entry=0x207f070,call=0x207f07c,offset=0x10,object_offset=0))
if a.subobject:
 specs.extend([
  dict(name='constructed-subobject-slot-20',assignment_entry=0x207bbf0,lea=0x207bd2e,adjust=None,store=0x207bd35,call_entry=0x1251290,call=0x1251434,offset=0x20,object_offset=0x458),
  dict(name='subobject-factory-allocate-slot-58',assignment_entry=0x207bbf0,lea=0x207bd2e,adjust=None,store=0x207bd35,call_entry=0x207bbf0,call=0x207bdfc,offset=0x58,object_offset=0x458),
  dict(name='subobject-factory-lock-slot-18',assignment_entry=0x207bbf0,lea=0x207bc5b,adjust=0x207bc62,store=0x207bc66,call_entry=0x207bbf0,call=0x207bcbc,offset=0x18,object_offset=0),
  dict(name='subobject-factory-lock-slot-28',assignment_entry=0x207bbf0,lea=0x207bc5b,adjust=0x207bc62,store=0x207bc66,call_entry=0x207bbf0,call=0x207bdab,offset=0x28,object_offset=0)])
proposals=[]
for spec in specs:
 ...
 assert store.operands[0].mem.disp==spec['object_offset']
