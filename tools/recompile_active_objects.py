...
def compile_one(row):
 ...
 for site in row.get('sites',[]):
  binding=selectors[bases[site['module']]+site['rva']];assert binding['lifted'] and ('BB_COMISS' if site['mnemonic'].endswith('ss') else 'BB_COMISD') in binding['implementation']
