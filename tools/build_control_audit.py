"""Build a read-only LLVM control-site inspector from the pinned configured libraries."""
...
source=ROOT/'native/control_audit/main.cpp';env=environment();steps=[]
for name,argv in [('compile',[LLVM/'bin/clang-cl.exe',*flags,'/c',source,'/Fo'+str(out/'control-audit.obj')]),('link',[LLVM/'bin/lld-link.exe',out/'control-audit.obj',*libs,'/machine:x64','/subsystem:console','/threads:2','/out:'+str(out/'control-audit.exe')])]:
...
write_json(out/'identity.json',dict(source_sha256=sha(source),executable_sha256=sha(out/'control-audit.exe'),compiler_sha256=sha(LLVM/'bin/clang-cl.exe'),linked_library_sha256=identities,semantics_sha256=sha(ROOT/'build/remill/lib/Arch/X86/Runtime/amd64_avx.bc'),execution='none'))
print('Read-only LLVM control-site inspector built',flush=True)
