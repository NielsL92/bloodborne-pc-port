"""Validate guest MXCSR loads/stores and explicit reserved-bit faults."""
import argparse,json,struct,subprocess
from pathlib import Path
from capstone import Cs,CS_ARCH_X86,CS_MODE_64
from tools.cfg_recover_startup import sha,write_json
from tools.dev import ROOT,LLVM,environment
p=argparse.ArgumentParser();p.add_argument('out',type=Path);p.add_argument('lifter',type=Path);p.add_argument('semantics',type=Path);p.add_argument('--prepare-only',action='store_true');p.add_argument('--characterize',action='store_true');a=p.parse_args()
out=a.out.resolve();out.mkdir(parents=True,exist_ok=False);env=environment();steps=[]
cases=[('load',['ldmxcsr [ARG]']),('store',['stmxcsr [ARG]']),('vload',['vldmxcsr [ARG]']),('vstore',['vstmxcsr [ARG]']),('load-store',['ldmxcsr [ARG]','stmxcsr [ARG]']),('store-load',['stmxcsr [ARG]','ldmxcsr [ARG]'])]
...
run('fixture',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/DADDRESS_SIZE_BITS=64','/DHAS_FEATURE_AVX=1','/DHAS_FEATURE_AVX512=0','/clang:-mlong-double-80','/clang:-mno-avx','/I'+str(ROOT/'external/remill/include'),'/I'+str(out),'/c',ROOT/'native/semantics/mxcsr_fixture.cpp','/Fo'+str(out/'fixture.obj')])
...
summary=dict(status='MXCSR instruction characterization complete',cases=sum(r['cases'] for r in observations),differing_cases=sum(r['differing_cases'] for r in observations),groups=observations,raw_sha256=sha(out/'execute.stdout'),lifter_sha256=sha(a.lifter),semantics_sha256=sha(a.semantics/'amd64_avx.bc'),fixture_sha256=sha(out/'fixture.exe'),limitations='Authored AOT only, no game bytes. Intel hardware supplies the 0xffff-profile oracle; 0xffbf mask exclusions are explicit policy cases. Invalid CSR loads fault before State updates; unavailable fixture memory is a separate unsupported diagnostic. Native runtime profile, memory and console exception delivery remain integration obligations.')
write_json(out/'summary.json',summary)
print(json.dumps({k:v for k,v in summary.items() if k!='groups'}),flush=True)
if not a.characterize:assert summary['differing_cases']==0, 'MXCSR contract differences retained in summary.json'
