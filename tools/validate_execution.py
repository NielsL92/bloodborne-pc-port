...
run_status=collections.Counter();active=[]
for p in (root/"local/runs").glob("*/manifest.json"):
 value=json.loads(p.read_text(encoding="utf-8-sig"));run_status[value["status"]]+=1
 if value["status"]=="running" and p.parent.name!="20260905-execution-audit":active.append(p.parent.name)
capture_summary=[]
