"""Validate scoped x87 FSCALE and precise completion boundaries."""
...
cases=[('scale',['fscale']),('scale-wait',['fscale','fwait']),('scale-sign',['fscale','fchs']),('scale-exchange-scale',['fscale','fxch st(1)','fscale']),('scale-store-pop',['fscale','fstp st(1)'])];indices=[1]*len(cases)
...
run('fixture',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/DADDRESS_SIZE_BITS=64','/DHAS_FEATURE_AVX=1','/DHAS_FEATURE_AVX512=0','/clang:-mlong-double-80','/clang:-mno-avx','/I'+str(ROOT/'external/remill/include'),'/I'+str(out),'/c',ROOT/'native/semantics/x87_scale_fixture.cpp','/Fo'+str(out/'fixture.obj')])
...
summary=dict(status='experimental x87 FSCALE characterization complete',cases=sum(r['cases'] for r in observations),differing_cases=sum(r['differing_cases'] for r in observations),groups=observations,library_sha256=sha(a.library),raw_sha256=sha(out/'execute.stdout'),lifter_sha256=sha(a.lifter),semantics_sha256=sha(a.semantics/'amd64_avx.bc'),fixture_sha256=sha(out/'fixture.exe'),limitations='Authored AOT and Intel hardware only; no game bytes. FSCALE uses scoped native integer exponent/rounding handling and SoftFloat NaN propagation; stack, completion and pending exceptions are instruction-level contracts. All PC values are included. Undefined C0/C2/C3 are excluded from the hardware status oracle. Pointer/opcode profiles are explicit. Native runtime integration remains open; absent MMX is explicitly rejected by the compiler.')
write_json(out/'summary.json',summary)
print(json.dumps({k:v for k,v in summary.items() if k!='groups'}),flush=True)
assert summary['differing_cases']==0, 'x87 FSCALE contract differences retained in summary.json'
