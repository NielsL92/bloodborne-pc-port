...
p=argparse.ArgumentParser();p.add_argument('source',type=Path);p.add_argument('out',type=Path);p.add_argument('--subobject',action='store_true');a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False)
...
for entry,site in calls:
 ...
 rows=cache[entry];position=next(n for n,r in enumerate(rows) if r[0]==site)
 widths=[13,14] if a.subobject else [12];original=[];ins=[];ok=False
 for width in widths:
  trial=rows[max(0,position-width+1):position+1];decoded=[next(d.disasm(bytes.fromhex(b),pc,count=1)) for pc,size,b in trial]
  if len(decoded)!=width:continue
  normal=list(decoded);valid=True
  if a.subobject:
   add=normal[6];valid=add.mnemonic=='add' and add.operands[0].type==X86_OP_REG and add.operands[0].size==8 and add.operands[1].type==X86_OP_IMM and add.operands[1].imm==0x458 and add.operands[0].reg==normal[5].operands[0].reg
   if width==14:
    first,reload=normal[0],normal[7]
    valid=valid and first.mnemonic==reload.mnemonic=='lea' and first.operands[0].type==reload.operands[0].type==X86_OP_REG and first.operands[0].reg==reload.operands[0].reg and first.operands[1].type==reload.operands[1].type==X86_OP_MEM and first.operands[1].mem.base==reload.operands[1].mem.base==X86_REG_RIP and not first.operands[1].mem.index and not reload.operands[1].mem.index and first.address+first.size+first.operands[1].mem.disp==reload.address+reload.size+reload.operands[1].mem.disp
    normal.pop(7)
   else:valid=valid and normal[0].reg_name(normal[0].operands[0].reg) in ('rbx','rbp','r12','r13','r14','r15')
   normal.pop(6)
  if valid and [i.mnemonic for i in normal]==['lea','mov','test','jne','call','mov','mov','mov','lea','xor','mov','call'] and all(x.address+x.size==y.address for x,y in zip(decoded,decoded[1:])):
   original=trial;ins=normal;ok=True;break
 if ok:
  ...
  ok=(lea.operands[0].type==X86_OP_REG and lea.operands[0].size==8 and lea.operands[1].type==X86_OP_MEM and lea.operands[1].mem.base==X86_REG_RIP and not lea.operands[1].mem.index
      ...
      and call.operands[0].type==X86_OP_IMM and call.operands[0].imm==(0x207bbf0 if a.subobject else 0x2ba2b10)
 ...
 matched.append(dict(module=main,entry=entry,site=site,start=lea.address,end=dispatch.address+dispatch.size,cache_slot=slot,factory=(0x207bbf0 if a.subobject else 0x2ba2b10),offset=0x20,object_offset=(0x458 if a.subobject else 0),registers=dict(cache=lea.reg_name(cache_reg),object=lea.reg_name(object_reg),table=lea.reg_name(table_reg)),instructions=[dict(pc=pc,size=size,bytes=b) for pc,size,b in rows],instruction_bytes_sha256=hashlib.sha256(b''.join(bytes.fromhex(b) for pc,size,b in rows)).hexdigest(),condition='Only a cache value produced by the checked factory with the stated object offset and retaining the constructed initial table. Mutation, interposition, failed initialization and native ABI remain unknown.'))
