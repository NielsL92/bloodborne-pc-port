"""Independently isolate x87 unmasked comparison flags without AOT or SEH."""
...
for form,mnemonic in enumerate(['fcomi','fcomip','fucomi','fucomip']):
 for setup in range(2):
  ...
  assembly.extend(['fldcw word ptr [rcx]','push qword ptr [rcx+8]','popfq','pushfq','pop r10','mov [r8+512],r10',f'{mnemonic} st(0), st(1)','pushfq','pop r10','mov [r8+520],r10','fxsave64 [r8]','fnclex','fxrstor64 [r9]','ret'])
...
run('fixture',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/clang:-mno-avx','/I'+str(out),'/c',ROOT/'native/semantics/x87_compare_probe.cpp','/Fo'+str(out/'fixture.obj')])
...
rows=[json.loads(line) for line in (out/'execute.stdout').open()];assert len(rows)==65536
invalid=[r for r in rows if (r['status']&1) and not(r['control']&1)]
summary=dict(status='independent authored hardware comparison completed',cases=len(rows),unmasked_invalid_cases=len(invalid),unmasked_invalid_unordered_flags=sum((r['after']&0x45)==0x45 for r in invalid),unmasked_invalid_flags_changed=sum((r['before']&0x45)!=(r['after']&0x45) for r in invalid),raw_sha256=sha(out/'execute.stdout'),fixture_sha256=sha(out/'fixture.exe'),assembly_sha256=sha(out/'authored.s'),source_sha256=sha(ROOT/'native/semantics/x87_compare_probe.cpp'),limitations='Authored native hardware only. FNINIT/FLD80/FLDCW initializes each case; immediate PUSHFQ captures flags before any waiting instruction. No Remill, AOT, FXRSTOR-imported guest values or SEH in comparison path. Host backup is restored before returning. Intel host observation does not settle AMD Jaguar semantics.')
