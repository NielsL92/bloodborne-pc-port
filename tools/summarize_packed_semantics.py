...
base=root/'local/compiler-spike/startup-batch-all-v1';left_dir=root/'local/compiler-spike/startup-transfer-recompile-v1';right_dir=root/'local/compiler-spike/startup-transfer-recompile-v2-repeat'
...
assert len(new_units)==41 and len(objects)==11 and len(traps)==16 and len(failures)==7 and not new_units&old_units
sem_dir=root/'build/extended-semantics-v6-transfer';sem=read(sem_dir/'identity.json');sem_hash=sha(sem_dir/'amd64_avx.bc');assert sem_hash==sem['semantics_sha256']
...
checks={}
for family,directory,count in [('packed','packed-semantics-v2-transfer',262144),('select','select-semantics-v2-transfer',376832),('transfer','transfer-semantics-v2',593408),('shuffle','shuffle-semantics-v3-transfer',32768),('blend','blend-semantics-v2-transfer',122880),('rsqrt','rsqrt-semantics-v2-transfer',399360),('bmi','bmi-transfer-regression-v1',34816)]:
 check=read(root/'local/compiler-spike'/directory/'summary.json');assert check['semantics_sha256']==sem_hash and check['result']['aot_cases']==count;checks[family]=check
runs={}
for suffix in ('build-extended-semantics-v4-packed','build-extended-semantics-v5-packed','packed-semantics-v1','select-semantics-v1','startup-packed-recompile-v1','build-extended-semantics-v6-transfer','transfer-semantics-v1','transfer-semantics-v2','select-semantics-v2-transfer','packed-semantics-v2-transfer','rsqrt-semantics-v2-transfer','blend-semantics-v2-transfer','shuffle-semantics-v3-transfer','bmi-transfer-regression-v1','startup-transfer-recompile-v1','startup-transfer-recompile-v2-repeat'):
 name='20260905-p3-'+suffix;p=root/'local/runs'/name;m=read(p/'manifest.json');assert m['status']==('failed' if suffix.startswith('startup-') or suffix in ('build-extended-semantics-v4-packed','transfer-semantics-v1') else 'pass')
