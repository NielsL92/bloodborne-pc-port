"""Characterize pending and newly raised x87 exceptions with authored code."""
...
cases.extend((cases[i][0]+'-wait',cases[i][1]+['fwait']) for i in [0,1,10,12,14,8,5])
...
header.append('static unsigned base_forms[]={'+','.join(map(str,list(range(19))+[0,1,10,12,14,8,5]))+'};')
...
run('fixture',[LLVM/'bin/clang-cl.exe','/nologo','/O2','/EHsc','/std:c++17','/DADDRESS_SIZE_BITS=64','/DHAS_FEATURE_AVX=1','/DHAS_FEATURE_AVX512=0','/clang:-mlong-double-80','/clang:-mno-avx','/I'+str(ROOT/'external/remill/include'),'/I'+str(out),'/c',ROOT/'native/semantics/x87_fault_probe.cpp','/Fo'+str(out/'fixture.obj')])
...
summary=dict(status='x87 pending and newly raised exception characterization complete',cases=sum(r['cases'] for r in observations),differing_cases=sum(r['differing_cases'] for r in observations),groups=observations,raw_sha256=sha(out/'execute.stdout'),lifter_sha256=sha(a.lifter),semantics_sha256=sha(a.semantics/'amd64_avx.bc'),fixture_sha256=sha(out/'fixture.exe'),limitations='Authored AOT and Intel host instructions only. Windows SEH captures hardware faults; an authored System V nonlocal frame captures explicit native callbacks. This does not implement Windows or PS4 guest exception delivery. Defined status, nonempty payloads, flags, fault instruction address and unaffected State compared. Guest last-IP/DP/FOP, reserved fields and undefined condition bits excluded; AMD Jaguar behavior unverified.')
