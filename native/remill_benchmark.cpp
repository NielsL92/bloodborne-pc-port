...
int main(int argc,char**argv) {
 ...
 auto code=static_cast<uint8_t*>(VirtualAlloc(nullptr,0x10000,MEM_RESERVE|MEM_COMMIT,PAGE_READWRITE));
 ...
 std::memcpy(code,bytes.data(),bytes.size());FlushInstructionCache(GetCurrentProcess(),code,0x10000);
 auto direct=reinterpret_cast<Direct>(code);constexpr int N=1000000;
 for(int length:{1,8,64}) {
  for(int i=0;i<length;++i) {
   auto node=arena+i*256;
   put(node,0x38,i+1<length?uint64_t(node+256):0);put(node,0x6a,uint8_t(i==length-1?0x80:0));
   put(node,0x20,uint64_t(arena+0x8000));put(arena+0x8000,0x10,uint64_t(0x1122334455667788ULL));
  }
  CpuFrame input{};input.gpr[7]=uint64_t(stack+0x8000-8);input.mxcsr=0x1f80;
  put(stack,0x8000-8,uint64_t(0xfeedbabe));
  State state;to_state(input,state);Memory memory{{{uint64_t(arena),0x10000},{uint64_t(stack),0x10000}},0};
  for(int rep=-1;rep<5;++rep) {
   ...
   for(int order=0;order<2;++order) {
    ...
    if(aot)for(int j=0;j<N;++j) {
     ...
     sub_100003360(&state,0x100003360,&memory);sum2+=state.gpr.rax.qword;
