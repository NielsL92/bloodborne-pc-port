...
extern "C" void __bb_native_x87_check_span(Memory* m,State* s,uint64_t at,uint32_t size,uint32_t write,uint32_t alignment){identity(m,s);++m->spans;if(at!=uint64_t(operand(m))||size!=store_widths[active_form][m->spans-1]||alignment!=1||write>1)fail("raw span request");if(!m->available){native_fault=2;__builtin_longjmp(escape,1);}}
...
static void run_case(unsigned form,unsigned top,unsigned tags,unsigned type,unsigned setting,unsigned mask,unsigned sticky,unsigned eflags,const uint8_t* original,bool unavailable=false,bool exception_only=false,const Value* override_source=nullptr){
 active_form=form;guest={};Memory& m=guest;m.available=!unavailable;m.exception_only=exception_only;m.offset=setting%8;m.stack[4]=0xfeed0001;for(unsigned i=0;i<64;++i)m.bytes[i]=uint8_t(0xa5+i*17+setting);uint8_t* data=operand(&m);
 ...
 for(unsigned st=0;st<8;++st){auto v=st==0&&override_source?*override_source:source_value(type+st);std::memcpy(&s.st.elems[st].val,&v.sig,8);put16(reinterpret_cast<uint8_t*>(&s.st.elems[st].val)+8,v.exp);std::memcpy(input+32+st*16,&s.st.elems[st].val,10);}
...
int main(){verify_numeric_scope();alignas(16) uint8_t original[512];save_host(original);
 ...
 for(unsigned profile=0;profile<2;++profile){
  for(unsigned top=0;top<8;++top)for(unsigned occupancy=0;occupancy<4;++occupancy)for(unsigned type=0;type<72;++type)for(unsigned mode=0;mode<8;++mode)for(unsigned flags=0;flags<8;++flags){unsigned masks[]={63,62,55,47,31,0,39,15};unsigned tags=occupancy==0?0:occupancy==1?255:1<<((top+occupancy-2)%8);run_case(form,top,tags,type,(mode+flags)%16,masks[mode],0,flags,original,false,profile!=0);}
  ...
  for(unsigned sample=72;sample<4168;++sample)run_case(form,sample%8,255,sample,sample%16,63,0,sample%8,original,false,profile!=0);
  for(unsigned sample=0;sample<8192;++sample){auto bits=random_bits(sample+0xa501);unsigned width=store_widths[form][0];unsigned lo=width==4?0x3f60:0x3bc0,range=width==4?0x130:0x860;Value value{random_bits(bits)|0x8000000000000000ULL,uint16_t((lo+sample%range)|((bits&1)<<15))};run_case(form,sample%8,255,2,sample%16,63,0,sample%8,original,false,profile!=0,&value);}
  const unsigned edge_exponents[]={0,1,0x3bcb,0x3bcc,0x3bcd,0x3bce,0x3c00,0x3c01,0x3c02,0x3f68,0x3f69,0x3f6a,0x3f6b,0x3f80,0x3f81,0x3f82,0x3ffe,0x3fff,0x4000,0x400d,0x400e,0x401d,0x401e,0x403d,0x403e,0x407e,0x407f,0x43fe,0x43ff,0x7ffe};
  const unsigned edge_masks[]={63,55,47,31,0};unsigned edge_case=0;
  for(unsigned discard:{11u,40u})for(unsigned exp:edge_exponents)for(unsigned base=0;base<3;++base)for(unsigned low=0;low<6;++low)for(unsigned sign=0;sign<2;++sign)for(unsigned mode=0;mode<4;++mode)for(unsigned mask:edge_masks){
   uint64_t unit=uint64_t(1)<<discard,half=unit>>1,offsets[]={0,1,half-1,half,half+1,unit-1};uint64_t upper=base==0?0x8000000000000000ULL:base==1?0x8000000000000000ULL+unit:~(unit-1);Value value{upper+offsets[low],uint16_t(exp|(sign<<15))};run_case(form,edge_case%8,255,2,mode+(edge_case%4)*4,mask,0,edge_case%8,original,false,profile!=0,&value);++edge_case;
  }
