...
static void run_case(unsigned form,unsigned top,unsigned tags,unsigned type,unsigned setting,unsigned mask,unsigned sticky,unsigned eflags,const uint8_t* original,bool unavailable=false,bool exception_only=false){
 active_form=form;guest={};Memory& m=guest;m.available=!unavailable;m.exception_only=exception_only;m.offset=setting%8;m.stack[4]=0xfeed0001;for(unsigned i=0;i<64;++i)m.bytes[i]=uint8_t(0xa5+i*17+setting);uint8_t* data=operand(&m);std::memcpy(data,&significands[type],8);put16(data+8,exponents[type]);
...
int main(){alignas(16) uint8_t original[512];save_host(original);
 ...
 for(unsigned profile=0;profile<2;++profile){
  ...
  if(base_forms[form]==0||base_forms[form]==1)for(unsigned pending=0;pending<2;++pending)run_case(form,0,255,2,3,pending?62:63,pending,0,original,true,profile!=0);
 }
