...
int main(){verify_numeric_scope();alignas(16) uint8_t original[512];save_host(original);
 ...
 for(unsigned profile=0;profile<2;++profile){
  for(unsigned top=0;top<8;++top)for(unsigned occupancy=0;occupancy<4;++occupancy)for(unsigned type=0;type<48;++type)for(unsigned mode=0;mode<8;++mode)for(unsigned flags=0;flags<2;++flags){unsigned masks[]={63,62,61,55,47,31,0,60};unsigned tags=occupancy==0?0:occupancy==1?255:1<<((top+(occupancy==2?0:source_indices[form]))%8);run_case(form,top,tags,type,(mode+flags+type)%12,masks[mode],0,flags?7:0,original,false,profile!=0);}
  for(unsigned top=0;top<8;++top)for(unsigned tags=0;tags<256;++tags)for(unsigned flags=0;flags<2;++flags)run_case(form,top,tags,tags%48,tags%12,flags?62:63,0,flags?7:0,original,false,profile!=0);
  for(unsigned sample=48;sample<4144;++sample)run_case(form,sample%8,255,sample,sample%12,63,0,sample%8,original,false,profile!=0);
  for(unsigned sample=0;sample<128;++sample){auto bits=random_bits(sample+0xa501);unsigned exponent=1+unsigned(bits%0x7ffe),other=1+unsigned(random_bits(bits)%0x7ffe);switch(sample%8){case 0:exponent=1;other=1;break;case 1:exponent=0x7ffe;other=0x7ffe;break;case 2:other=exponent;break;case 3:exponent=0x7ffe;other=1;break;case 4:exponent=1;other=0x3ffe;break;case 5:exponent=1;other=0x4000;break;case 6:exponent=0x7ffe;other=0x3fff;break;default:break;}Value a{random_bits(bits)|0x8000000000000000ULL,uint16_t(exponent|((bits&1)<<15))},b{random_bits(bits+1)|0x8000000000000000ULL,uint16_t(other|((bits&2)<<14))};
   for(unsigned mode=0;mode<12;++mode)for(unsigned mask:{63u,62u,61u,55u,47u,31u,0u,60u})run_case(form,sample%8,255,2,mode,mask,0,sample%8,original,false,profile!=0,&a,&b);
  }
  for(unsigned masks=0;masks<64;++masks)for(unsigned sticky=0;sticky<64;++sticky)run_case(form,sticky%8,255,2,masks%12,masks,sticky,masks%8,original,false,profile!=0);
  for(unsigned top=0;top<8;++top)for(unsigned occupancy=0;occupancy<4;++occupancy)for(unsigned masks=0;masks<64;++masks)for(unsigned sticky:{0u,1u,2u,8u,16u,32u})run_case(form,top,occupancy==0?0:occupancy==1?255:1<<((top+(occupancy==2?0:source_indices[form]))%8),2,3,masks,sticky,0,original,true,profile!=0);
