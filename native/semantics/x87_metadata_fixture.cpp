...
struct Memory {uint64_t stack[8];uint8_t bytes[64];unsigned offset=0,reads=0,writes=0,returns=0,spans=0,records=0,policies=0,resets=0,comparison_profile=0;bool exception_only=false,available=true;uint32_t segments=0x00770066;};
...
extern "C" void __bb_native_x87_check_span(Memory* m,State* s,uint64_t at,uint32_t size,uint32_t write,uint32_t alignment){identity(m,s);++m->spans;if(at!=uint64_t(operand(m))||size!=10||alignment!=1||write>1)fail("raw span request");if(!m->available){native_fault=2;__builtin_longjmp(escape,1);}}
extern "C" uint64_t __bb_native_x87_image_policy(Memory* m,State* s){identity(m,s);++m->policies;return (m->exception_only?12:0)|((m->comparison_profile&1)?16:0)|((m->comparison_profile&2)?32:0);}
...
extern "C" void __bb_native_x87_set_pointer_segments(Memory* m,State* s,uint32_t value){identity(m,s);if(value||s->x87.fxsave.ip||s->x87.fxsave.dp||s->x87.fxsave.fop)fail("initialization metadata reset order");++m->resets;m->segments=value;}
...
static const uint64_t significands[]={0,0,0x8000000000000000ULL,0x8000000000000000ULL,0x8000000000000000ULL,0xc000000000000123ULL,0x8000000000000123ULL,1,0x7fffffffffffffffULL,0x8000000000000000ULL,0x1234,0,0x4000000000000000ULL,0xffffffffffffffffULL,0x8000000000000001ULL,0x8000000000000000ULL};
static const uint16_t exponents[]={0,0x8000,0x3fff,0xbfff,0x7fff,0x7fff,0x7fff,0,0,0,0x3fff,0x7fff,0x7fff,0x7ffe,0x8000,0xffff};
...
static uint64_t counts[form_count]{},bad[form_count]{},findings[form_count][10]{},hardware_faults[form_count]{},native_faults[form_count]{},unsupported_cases[form_count]{},hardware_pointer_differences[form_count]{},hardware_opcode_differences[form_count]{},selected_c1_deviations[form_count]{},selected_aux_flag_deviations[form_count]{};
static void run_case(unsigned form,unsigned top,unsigned tags,unsigned type,unsigned setting,unsigned mask,unsigned sticky,unsigned eflags,const uint8_t* original,bool unavailable=false,bool exception_only=false,unsigned comparison_profile=0){
 active_form=form;guest={};Memory& m=guest;m.available=!unavailable;m.exception_only=exception_only;m.comparison_profile=comparison_profile;m.offset=setting%8;m.stack[4]=0xfeed0001;for(unsigned i=0;i<64;++i)m.bytes[i]=uint8_t(0xa5+i*17+setting);uint8_t* data=operand(&m);std::memcpy(data,&significands[type%16],8);put16(data+8,exponents[type%16]);
 ...
 for(unsigned st=0;st<8;++st){unsigned t=st==0?type%16:st==1||st==7?(type/16+1)%16:(type+st)%16;std::memcpy(&s.st.elems[st].val,&significands[t],8);put16(reinterpret_cast<uint8_t*>(&s.st.elems[st].val)+8,exponents[t]);std::memcpy(input+32+st*16,&s.st.elems[st].val,10);}
 ...
 unsigned initial_pending=sticky&~mask&63,expected_fault=unavailable?(initial_pending?1:2):fault.code?1:0;
 // Unavailable cases are intentionally selected only for memory-first forms.
 unsigned reads=0,writes=0,spans=0,records=0,policies=0,resets=0;uint32_t segments=0x00770066;uint64_t last_pc=expected.x87.fxsave.ip,last_dp=expected.x87.fxsave.dp;uint16_t last_fop=expected.x87.fxsave.fop;int last_step=-1;unsigned expected_c1=get16(input+2)&0x200;bool had_compare=false;
 if(unavailable){if(!initial_pending)spans=1;}
 else for(unsigned j=0;j<instruction_counts[form];++j){
  unsigned kind=unsigned(step_kinds[form][j]);if(fault.code&&step_pcs[form][j]>=fault.pc)break;if(!kind)continue;if(kind==7){expected_c1=0;++resets;last_step=int(j);last_pc=last_dp=last_fop=0;segments=0;continue;}const uint8_t* prior_state=j?observed+1072+(j-1)*512:input;
  ++records;++policies;last_step=int(j);last_pc=step_pcs[form][j];const uint8_t* after_opcode=observed+1072+j*512;expected_c1=get16(after_opcode+2)&0x200;if(kind==6){had_compare=true;if(!(comparison_profile&1))expected_c1=0;}if(!exception_only||(get16(after_opcode+2)&~get16(after_opcode)&63))last_fop=uint16_t(step_fops[form][j]);segments=(segments&0xffff0000)|0x43;
  if(kind==1||kind==2){++spans;const uint8_t* after_state=observed+1072+j*512;bool update=!exception_only||(get16(after_state+2)&~get16(after_state)&63);if(update){last_dp=uint64_t(data);segments=0x004b0043;}if(kind==1)reads+=10;else{unsigned t=(get16(prior_state+2)>>11)&7;bool empty=!(prior_state[4]&(1<<t));if(!empty||(get16(prior_state)&1))writes+=10;}}
 }
 unsigned other_cc=s.x87.fxsave.swd.flat&0x4500;
 if(last_step>=0){unsigned kind=unsigned(step_kinds[form][last_step]);if(kind==7)other_cc=0;else if(kind==6){const uint8_t* prior_step=last_step?observed+1072+(last_step-1)*512:input;other_cc=get16(prior_step+2)&0x4500;if((get16(observed+2)&0x4500)!=other_cc)fail("hardware comparison changed documented preserved condition bits");}}
 if(!unavailable){
  expected.x87.fxsave.cwd.flat=get16(observed);status(expected,uint16_t((get16(observed+2)&~0x4700)|other_cc|expected_c1));expected.x87.fxsave.ftw.flat=observed[4];
  for(unsigned st=0;st<8;++st)std::memcpy(&expected.st.elems[st].val,observed+32+st*16,10);
 }
 expected.aflag.cf=get16(observed+544)&1;expected.aflag.pf=(get16(observed+544)>>2)&1;expected.aflag.af=(get16(observed+544)>>4)&1;expected.aflag.zf=(get16(observed+544)>>6)&1;expected.aflag.sf=(get16(observed+544)>>7)&1;expected.aflag.of=(get16(observed+544)>>11)&1;
 if(had_compare&&(comparison_profile&2)){expected.aflag.af=(initial_flags>>4)&1;expected.aflag.sf=(initial_flags>>7)&1;expected.aflag.of=(initial_flags>>11)&1;}
 expected.x87.fxsave.ip=last_pc;expected.x87.fxsave.dp=last_dp;expected.x87.fxsave.fop=last_fop;
 expected.gpr.rip.qword=expected_fault?(unavailable?pcs[form]:fault.pc):0xfeed0001;if(!expected_fault)expected.gpr.rsp.qword+=8;
 bool differences[]={import_difference,std::memcmp(m.bytes,unavailable?prior:hw_bytes,64)!=0,std::memcmp(&s,&expected,sizeof s)!=0,std::memcmp(before,after,28)!=0||std::memcmp(before+32,after+32,128)!=0,!unavailable&&flags(s)!=flags(expected),native_fault!=expected_fault,s.gpr.rip.qword!=expected.gpr.rip.qword,native_fault==1&&native_pending!=(unavailable?initial_pending:get16(observed+2)&~get16(observed)&63),m.spans!=spans||m.reads!=reads||m.writes!=writes||m.records!=records||m.policies!=policies||m.returns!=unsigned(!expected_fault)||(!expected_fault&&returned_memory!=&m),m.segments!=segments||m.resets!=resets};
 ...
 if(!unavailable){selected_c1_deviations[form]+=expected_c1!=(get16(observed+2)&0x200);selected_aux_flag_deviations[form]+=(flags(expected)&0x890)!=(get16(observed+544)&0x890);}
...
int main(){alignas(16) uint8_t original[512];save_host(original);
 ...
 for(unsigned profile=0;profile<8;++profile){
  for(unsigned top=0;top<8;++top)for(unsigned occupancy=0;occupancy<4;++occupancy)for(unsigned type=0;type<16;++type)for(unsigned mode=0;mode<4;++mode)for(unsigned flags=0;flags<8;++flags){unsigned masks[]={63,62,61,0};unsigned tags=occupancy==0?0:occupancy==1?255:1<<((top+occupancy-2)%8);run_case(form,top,tags,type,(mode+flags)%8,masks[mode],0,flags,original,false,(profile&1)!=0,profile>>1);}
  for(unsigned top=0;top<8;++top)for(unsigned tags=0;tags<256;++tags)for(unsigned flags=0;flags<2;++flags)run_case(form,top,tags,tags%16,tags%8,flags?62:63,0,flags?7:0,original,false,(profile&1)!=0,profile>>1);
  for(unsigned masks=0;masks<64;++masks)for(unsigned sticky=0;sticky<64;++sticky)run_case(form,sticky%8,255,2,masks%8,masks,sticky,masks%8,original,false,(profile&1)!=0,profile>>1);
  if(base_forms[form]>=7&&base_forms[form]<=10)for(unsigned pair=0;pair<256;++pair)for(unsigned mask=0;mask<4;++mask)for(unsigned flag=0;flag<8;++flag)run_case(form,0,255,pair,flag,63^mask,0,flag,original,false,(profile&1)!=0,profile>>1);
 }
  std::printf("{\"selected_c1_deviations\":%llu,\"selected_aux_flag_deviations\":%llu,\"form\":%u,\"cases\":%llu,\"differing_cases\":%llu,\"hardware_faults\":%llu,\"native_faults\":%llu,\"unsupported_cases\":%llu,\"pointer_observation_differences\":%llu,\"opcode_observation_differences\":%llu,\"differences\":{",(unsigned long long)selected_c1_deviations[form],(unsigned long long)selected_aux_flag_deviations[form],form,(unsigned long long)counts[form],(unsigned long long)bad[form],(unsigned long long)hardware_faults[form],(unsigned long long)native_faults[form],(unsigned long long)unsupported_cases[form],(unsigned long long)hardware_pointer_differences[form],(unsigned long long)hardware_opcode_differences[form]);for(unsigned i=0;i<10;++i)std::printf("%s\"%s\":%llu",i?",":"",labels[i],(unsigned long long)findings[form][i]);std::printf("}}\n");std::fflush(stdout);
