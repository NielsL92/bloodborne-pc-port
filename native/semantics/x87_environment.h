...
// guest profile selection. Not yet bound to startup instruction selectors.
#include <cstdint>
#include <cstring>
#include <remill/Arch/X86/Runtime/State.h>
namespace bb_x87 {
struct Bits {uint64_t significand;uint16_t sign_exponent;};
inline Bits bits(const float80_t& value){Bits out;std::memcpy(&out.significand,&value,8);std::memcpy(&out.sign_exponent,reinterpret_cast<const uint8_t*>(&value)+8,2);return out;}
inline void put_bits(float80_t& value,Bits in){std::memcpy(&value,&in.significand,8);std::memcpy(reinterpret_cast<uint8_t*>(&value)+8,&in.sign_exponent,2);}
inline uint16_t read16(const uint8_t* p){uint16_t v;std::memcpy(&v,p,2);return v;}
inline uint32_t read32(const uint8_t* p){uint32_t v;std::memcpy(&v,p,4);return v;}
inline void write16(uint8_t* p,uint16_t v){std::memcpy(p,&v,2);}
inline void write32(uint8_t* p,uint32_t v){std::memcpy(p,&v,4);}
inline void write64(uint8_t* p,uint64_t v){std::memcpy(p,&v,8);}
inline uint16_t normalize_control(uint16_t v){return uint16_t((v&0x1f3f)|0x40);}
inline uint16_t status(const State& s){uint16_t v=s.x87.fxsave.swd.flat&~uint16_t(0x477f);v|=(s.sw.ie&1)|((s.sw.de&1)<<1)|((s.sw.ze&1)<<2)|((s.sw.oe&1)<<3)|((s.sw.ue&1)<<4)|((s.sw.pe&1)<<5)|((s.sw.sf&1)<<6)|((s.sw.c0&1)<<8)|((s.sw.c1&1)<<9)|((s.sw.c2&1)<<10)|((s.sw.c3&1)<<14);return v;}
inline void set_status(State& s,uint16_t v){s.x87.fxsave.swd.flat=v;s.sw.ie=v&1;s.sw.de=(v>>1)&1;s.sw.ze=(v>>2)&1;s.sw.oe=(v>>3)&1;s.sw.ue=(v>>4)&1;s.sw.pe=(v>>5)&1;s.sw.sf=(v>>6)&1;s.sw.c0=(v>>8)&1;s.sw.c1=(v>>9)&1;s.sw.c2=(v>>10)&1;s.sw.c3=(v>>14)&1;}
inline uint16_t recompute_summary(uint16_t sw,uint16_t cw){return uint16_t((sw&~0x8080)|((sw&~cw&63)?0x8080:0));}
...
inline uint16_t full_tags(const State& s){unsigned top=(status(s)>>11)&7,word=0;for(unsigned physical=0;physical<8;++physical){unsigned tag=3;if(s.x87.fxsave.ftw.flat&(1<<physical)){auto v=bits(s.st.elems[(physical+8-top)&7].val);unsigned exponent=v.sign_exponent&0x7fff;tag=!exponent&&!v.significand?1:exponent&&exponent!=0x7fff&&(v.significand>>63)?0:2;}word|=tag<<(physical*2);}return uint16_t(word);}
inline void store_environment28(State& s,PointerSegments segments,const Profile& profile,uint8_t* dst){
...
inline PointerSegments load_environment28(State& s,const Profile& profile,const uint8_t* src){
...
inline void save_fx(State& s,PointerSegments segments,const Profile& profile,bool offset64,uint8_t* dst){
 ...
 for(unsigned i=0;i<8;++i){std::memcpy(dst+32+i*16,&s.st.elems[i].val,10);std::memset(dst+42+i*16,0,6);}
 for(unsigned i=0;i<16;++i)std::memcpy(dst+160+i*16,&s.vec[i].xmm,16);
