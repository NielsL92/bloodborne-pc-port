...
extern "C" void __bb_native_x87_check_span(Memory* m,State* s,uint64_t at,uint32_t size,uint32_t write,uint32_t alignment){identity(m,s);++m->spans;if(at!=uint64_t(operand(m))||size!=(write?10:load_widths[active_form])||alignment!=1||write>1)fail("load span request");if(!m->available){native_fault=2;__builtin_longjmp(escape,1);}}
...
extern "C" void __bb_native_x87_record_instruction(Memory* m,State* s,uint64_t pc,uint32_t memory,uint64_t at){
 identity(m,s);bool found=false;for(unsigned j=0;j<instruction_counts[active_form];++j)if(step_pcs[active_form][j]==pc){found=true;bool mem=step_kinds[active_form][j]==1||step_kinds[active_form][j]==2;bool update=mem&&(!m->exception_only||(s->x87.fxsave.swd.flat&~s->x87.fxsave.cwd.flat&63));if(memory!=(unsigned(mem)|(unsigned(update)<<1))||at!=(mem?uint64_t(operand(m)):0)||s->x87.fxsave.ip!=pc||((!m->exception_only||(s->x87.fxsave.swd.flat&~s->x87.fxsave.cwd.flat&63))&&s->x87.fxsave.fop!=step_fops[active_form][j])){std::fprintf(stderr,"metadata form=%u step=%u pc=%llx/%llx fop=%x/%llx flags=%x expected=%x addr=%llx/%llx cw=%x sw=%x\n",active_form,j,(unsigned long long)s->x87.fxsave.ip,(unsigned long long)pc,s->x87.fxsave.fop,(unsigned long long)step_fops[active_form][j],memory,(unsigned(mem)|(unsigned(update)<<1)),(unsigned long long)at,(unsigned long long)(mem?uint64_t(operand(m)):0),s->x87.fxsave.cwd.flat,s->x87.fxsave.swd.flat);fail("record instruction metadata");}}
...
static const uint32_t source32[]={0,0x80000000,0x3f800000,0xbf800000,0x7f800000,0xff800000,0x7fc00001,0xffc00001,0x7f800001,0xff800001,1,0x80000001,0x7fffff,0x807fffff,0x800000,0x80800000,0x7f7fffff,0xff7fffff,0x3f800001,0xbf800001,0x3f7fffff,0xbf7fffff,0x00800001,0x80800001,0x7fffffff,0xffffffff,0x55555555,0xaaaaaaaa,0x7fff,0x8000,0xffff,0x80000000};
static const uint64_t source64[]={0,0x8000000000000000ULL,0x3ff0000000000000ULL,0xbff0000000000000ULL,0x7ff0000000000000ULL,0xfff0000000000000ULL,0x7ff8000000000001ULL,0xfff8000000000001ULL,0x7ff0000000000001ULL,0xfff0000000000001ULL,1,0x8000000000000001ULL,0x000fffffffffffffULL,0x800fffffffffffffULL,0x0010000000000000ULL,0x8010000000000000ULL,0x7fefffffffffffffULL,0xffefffffffffffffULL,0x3ff0000000000001ULL,0xbff0000000000001ULL,0x3fefffffffffffffULL,0xbfefffffffffffffULL,0x0010000000000001ULL,0x8010000000000001ULL,0x7fffffffffffffffULL,0xffffffffffffffffULL,0x5555555555555555ULL,0xaaaaaaaaaaaaaaaaULL,0x7fff,0x8000,0xffff,0x80000000};
...
static void run_case(unsigned form,unsigned top,unsigned tags,unsigned type,unsigned setting,unsigned mask,unsigned sticky,unsigned eflags,const uint8_t* original,bool unavailable=false,bool exception_only=false){
 active_form=form;guest={};Memory& m=guest;m.available=!unavailable;m.exception_only=exception_only;m.offset=setting%8;m.stack[4]=0xfeed0001;for(unsigned i=0;i<64;++i)m.bytes[i]=uint8_t(0xa5+i*17+setting);uint8_t* data=operand(&m);uint64_t source=type<32?(load_widths[form]==8?source64[type]:source32[type]):random_bits(type);std::memcpy(data,&source,load_widths[form]);
 ...
 for(unsigned st=0;st<8;++st){unsigned t=(type+st)%16;std::memcpy(&s.st.elems[st].val,&significands[t],8);put16(reinterpret_cast<uint8_t*>(&s.st.elems[st].val)+8,exponents[t]);std::memcpy(input+32+st*16,&s.st.elems[st].val,10);}
...
int main(){verify_numeric_scope();alignas(16) uint8_t original[512];save_host(original);
 ...
 for(unsigned profile=0;profile<2;++profile){
  for(unsigned top=0;top<8;++top)for(unsigned occupancy=0;occupancy<4;++occupancy)for(unsigned type=0;type<32;++type)for(unsigned mode=0;mode<4;++mode)for(unsigned flags=0;flags<8;++flags){unsigned masks[]={63,62,61,0};unsigned tags=occupancy==0?0:occupancy==1?255:1<<((top+occupancy-2)%8);run_case(form,top,tags,type,mode+flags*4,masks[mode],0,flags,original,false,profile!=0);}
  for(unsigned top=0;top<8;++top)for(unsigned tags=0;tags<256;++tags)for(unsigned flags=0;flags<2;++flags)run_case(form,top,tags,tags%16,tags%8,flags?62:63,0,flags?7:0,original,false,profile!=0);
  for(unsigned sample=32;sample<4128;++sample)run_case(form,sample%8,0,sample,sample%16,63,0,sample%8,original,false,profile!=0);
  for(unsigned masks=0;masks<64;++masks)for(unsigned sticky=0;sticky<64;++sticky)run_case(form,sticky%8,255,2,masks%8,masks,sticky,masks%8,original,false,profile!=0);
  for(unsigned pending=0;pending<2;++pending)run_case(form,0,255,2,3,pending?62:63,pending,0,original,true,profile!=0);
 }
