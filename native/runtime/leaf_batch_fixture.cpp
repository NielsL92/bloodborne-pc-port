...
int main(int argc,char** argv){
 ...
 for(const auto& leaf:LEAVES)for(unsigned v=0;v<256;++v)for(uint64_t initial:{0ULL,0xffffffffffffffffULL,0x123456789abcdef0ULL,0x8000000000000055ULL})for(uint64_t offset:{0ULL,17ULL,256ULL-leaf.width-leaf.displacement}){
  uint64_t supplied=leaf.width==1?v:(uint64_t(v)*0x9e3779b97f4a7c15ULL)^initial;if(leaf.width)space.write(&m,DATA+offset+leaf.displacement,&supplied,leaf.width);uint64_t expected=leaf.width==1?initial/256*256+v:leaf.width==8?supplied:leaf.address;require(call(leaf,DATA+offset,initial,false,false)==expected);if(leaf.width){uint64_t after=0;space.read(&m,DATA+offset+leaf.displacement,&after,leaf.width);require(after==supplied);}
 }
