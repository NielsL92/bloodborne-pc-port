...
int main(int argc,char** argv){
 if(argc!=2)return 2;bb_runtime::AddressSpace space;space.add(DATA,256,3);space.add(STACK,256,3);space.guard(STACK+128,8,"leaf-return-unresolved");space.guard(DATA+128,1,"leaf-unresolved-byte");space.seal();State state{};Memory m{};m.state=&state;m.space=&space;m.tables=&tables;m.owner_thread=GetCurrentThreadId();m.entry=PC;bb_runtime::validate_tables(tables);uint64_t cases=0;
 auto call=[&](uint64_t at,uint64_t value){state=State{};state.gpr.rip.qword=PC;state.gpr.rsp.qword=LEAF_ADDRESS_FORM&&!std::strcmp(argv[1],"unmapped")?STACK+256:LEAF_ADDRESS_FORM&&!std::strcmp(argv[1],"guard")?STACK+128:STACK+248;state.gpr.rdi.qword=at;state.gpr.rax.qword=value;state.gpr.rbx.qword=0x123456789abcdef0;state.gpr.rbp.qword=0xfedcba9876543210;state.gpr.r12.qword=12;state.gpr.r13.qword=13;state.gpr.r14.qword=14;state.gpr.r15.qword=15;state.aflag.cf=1;state.aflag.pf=1;state.aflag.af=1;state.aflag.zf=1;state.aflag.sf=1;state.aflag.of=1;if(state.gpr.rsp.qword==STACK+248)space.write(&m,STACK+248,&RET,8);require(bb_runtime::dispatch(&state,PC,&m)==&m);require(state.gpr.rip.qword==RET&&state.gpr.rsp.qword==STACK+256&&state.gpr.rdi.qword==at);require(state.gpr.rbx.qword==0x123456789abcdef0&&state.gpr.rbp.qword==0xfedcba9876543210&&state.gpr.r12.qword==12&&state.gpr.r13.qword==13&&state.gpr.r14.qword==14&&state.gpr.r15.qword==15);require(state.aflag.cf&&state.aflag.pf&&state.aflag.af&&state.aflag.zf&&state.aflag.sf&&state.aflag.of);++cases;return state.gpr.rax.qword;};
 ...
 for(unsigned value=0;value<256;++value)for(uint64_t initial:{0ULL,0xffffffffffffffffULL,0x123456789abcdef0ULL,0x8000000000000055ULL})for(uint64_t offset:{0ULL,17ULL,256ULL-LEAF_WIDTH-LEAF_DISPLACEMENT}){
  if(LEAF_ADDRESS_FORM){require(call(DATA+offset,initial)==LEAF_ADDRESS);continue;}
