...
[[noreturn]] void fault(Memory* m,const char* boundary,uint32_t reason,uint64_t source,uint64_t actual,uint64_t wanted,uint64_t address,uint64_t width)noexcept{
 ...
 if(m&&m->active_source){s=m->active_source->state;source=m->active_source->pc;s->gpr.rip.qword=source;}
 ...
 if(m&&m->active_guard)std::fprintf(out,"{\"unresolved_identity\":\"%s\",\"guard_address\":\"%016llx\",\"guard_size\":%llu,\"completed_memory_operations\":%llu}\n",m->active_guard->identity.c_str(),(unsigned long long)m->active_guard->base,(unsigned long long)m->active_guard->size,(unsigned long long)m->operations);
